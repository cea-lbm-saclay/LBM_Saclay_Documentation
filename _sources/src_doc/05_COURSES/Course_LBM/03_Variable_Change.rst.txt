...
Dans ce qui suit, les calculs sont développés en posant :math:`\vartheta\equiv f`, :math:`\mathcal{S}_{i}^{\vartheta}=\mathcal{S}_{i}^{f}=\mathcal{S}_{i}`
et :math:`\tau_{\vartheta}\equiv\tau`, mais l’approche du changement de variable est également valable pour :math:`\vartheta\equiv h` et :math:`\vartheta\equiv s`. Les termes qui sont évalués en :math:`\boldsymbol{x}` et :math:`t` sont notés :math:`f_{i}\equiv f_{i}(\boldsymbol{x},\,t)`,
:math:`f_{i}^{eq}\equiv f_{i}^{eq}(\boldsymbol{x},\,t)` et :math:`\mathcal{S}_{i}\equiv\mathcal{S}_{i}(\boldsymbol{x},\,t)`, tandis
que les termes évalués en :math:`\boldsymbol{x}+\boldsymbol{c}_{i}\delta t` et :math:`t+\delta t` sont notés avec une étoile : :math:`f_{i}^{\star}\equiv f_{i}(\boldsymbol{x}+\boldsymbol{c}_{i}\delta t,\,t+\delta t)`, :math:`f_{i}^{\star eq}\equiv f_{i}^{eq}(\boldsymbol{x}+\boldsymbol{c}_{i}\delta t,\,t+\delta t)` et :math:`\mathcal{S}_{i}^{\star}\equiv\mathcal{S}_{i}(\boldsymbol{x}+\boldsymbol{c}_{i}\delta t,\,t+\delta t)`. Avec ces notations, l’intégration de l’Eq. :eq:`DLBE` entre :math:`t` et :math:`t+\delta t` donne :

.. math::
   :label: LBE-Trapezoidal
...
où la loi des trapèzes à été appliquée pour le membre de droite de l’Eq. :eq:`DLBE`. Dans cette expression, pour les termes en :math:`\mathbf{x}+\mathbf{c}_{i}\delta t` et :math:`t+\delta t`, on
...
Le même changement de variable est utilisé pour les termes en :math:`\boldsymbol{x}` et :math:`t` :

.. math::
   :label: VariableChange-Expl
...
.. math::
   :label: f-versus-fbarre
   
   f_{i}=\frac{2\tau}{2\tau+\delta t}\left(\overline{f}_{i}+\frac{\delta t}{2\tau}f_{i}^{eq}+\frac{\delta t}{2}\mathcal{S}_{i}\right)

Avec les Eqs. :eq:`VariableChange` et :eq:`f-versus-fbarre`, l’Eq. :eq:`LBE-Trapezoidal` devient

.. math::
   :label: LBE-Intermediate
...
.. math::
   :label: Feq-barre-Varchange
   
   \overline{f}_{i}^{eq}=f_{i}^{eq}-\frac{\delta t}{2}\mathcal{S}_{i}

alors l’Eq. :eq:`LBE-Intermediate` est équivalente à
...
:eq:`LBE-Intermediate` est équivalente à
...
:eq:`LBE-Form1` s’écrit finalement

.. math::
   :label: LBE-2ndOrderTime-Form1
...
:eq:`LBE-2ndOrderTime-Form1` est
l’équation d’évolution utilisée dans les algorithmes LBM. Le changement
de variable Eq. :eq:`VariableChange-Expl` conduit à
...
On montre ici l'équivalence entre l’Eq. 6.25 dans :footcite:p:`BookLBM2017` (page 239) et de l’Eq. :eq:`LBE-2ndOrderTime-Form2`
...
En l’utilisant dans :eq:`Def-Lambda`, on obtient :

.. math::

   \lambda &=\overline{\tau}\delta t+\frac{\delta t}{2}\nonumber \\
      &=\left(\overline{\tau}+1/2\right)\delta t

En remplaçant dans Eq. :eq:`Eq625`, on obtient :
...
qui correspond à l’Eq. :eq:`LBE-2ndOrderTime-Form2`.

Lien avec la formulation qui introduit :math:`\overline{f}_{i}^{eq}`
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

L’Eq. :eq:`Eq625` avec :math:`\lambda` défini par l’Eq. :eq:`Def-Lambda` s’écrit :
