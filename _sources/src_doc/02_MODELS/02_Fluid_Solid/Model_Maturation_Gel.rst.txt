...
The model of gel maturation is an example of multi-component phase-field model for two phases: solid and liquid and three compositions (ternary system). In section ``[lbm]``, the kernel is activated with the keyword ``problem=ternary_GP_mixt``. In the rest of this section, the mathematical model and input parameters are described for model option ``model=two_dilute``.
...
.. admonition:: Mathematical model
   ...
   In this model we are concerned with the diffusion of three components :math:`A`, :math:`B` and :math:`C` of compositions :math:`C_A`, :math:`C_B` and :math:`C_C`. It is derived in :footcite:p:`Teo_Boutin_PhD2025`. The derivation is based on the grand-potential formulation of phase-field model :footcite:p:`Plapp_GrandPotential_PRE2011`. The compositions are conserved and the following relationship holds:
...
.. sectionauthor:: Alain Cartalade & Téo Boutin
