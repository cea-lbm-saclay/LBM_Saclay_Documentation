...
.. math::
   :label: PoidsD2Q9-Chapman-Enskog
...
The purpose of this section is to demonstrate with the Chapman-Enskog expansion that Eq.  :eq:`LBE-BGK-Chapman-Enskog` with equilibrium  :eq:`Feq-Chapman-Enskog` and lattice D2Q9 (relations :eq:`DirE0-Chapman-Enskog` -- :eq:`DirE5-E8-Chapman-Enskog` and
:eq:`PoidsD2Q9-Chapman-Enskog`) simulate the low-Mach Navier-Stokes equations:

.. math::
   :label: Conversation-Masse-Chapman-Enskog

   \partial_{t}\rho+\partial_{\alpha}(\rho u_{\alpha})=0

.. math::
   :label: Conservation-Qmvt-Chapman-Enskog
...
Eq. :eq:`Conversation-Masse-Chapman-Enskog` is the mass balance equation and :eq:`Conservation-Qmvt-Chapman-Enskog` and the impulsion balance equation. In those equations, the greek indices are :math:`\alpha=x,y` and
...
:eq:`Conservation-Qmvt-Chapman-Enskog` :math:`p` is the (thermodynamic)
...
Without loss of generality, we set :math:`\Delta x=\Delta t=1` (lattice units) in order to simplify the notations. In general case, :math:`\Delta x` and :math:`\Delta t` can take values different of unity. We get in Eq. :eq:`Feq-Chapman-Enskog` :math:`c=1`. We also set:

.. math::
   :label: VitesseSon-Chapman-Enskog
...
.. math::
   :label: Feq-grecs-Chapman-Enskog
...
.. math::
   :label: LBE-BGK-Unite-Chapman-Enskog

   f_{i}(x+e_{i\beta},\,t+1)=f_{i}(x,\,t)-\frac{1}{\lambda}\left[f_{i}(x,\,t)-f_{i}^{(0)}(x,\,t)\right]

The Taylor's expansion of left-hand side of Eq. :eq:`LBE-BGK-Unite-Chapman-Enskog` up to second order in space and times yields:
...
By replacing in Eq. :eq:`LBE-BGK-Unite-Chapman-Enskog`, we obtain:

.. math::
   :label: LBE-Taylor-Chapman-Enskog
...
.. math::
   :label: ScalesPartial-Chapman-Enskog
...
.. math::
   :label: F-Expansion-Chapman-Enskog

   f_{i}\simeq f_{i}^{(0)}+\varepsilon f_{i}^{(1)}+\varepsilon f_{i}^{(2)}

That expansion and Eqs. :eq:`Def-F-Chapman-Enskog` and :eq:`DefF0-Chapman-Enskog` imply that:

.. math::
   :label: M0-fneq-Chapman-Enskog

   \sum_{i}f_{i}^{(1)}=\sum_{i}f_{i}^{(2)}=0

.. math::
   :label: M1-fneq-Chapman-Enskog

   \sum_{i}f_{i}^{(1)}e_{i\alpha}=\sum_{i}f_{i}^{(2)}e_{i\alpha}=0

**Remark**

Eq. :eq:`M1-fneq-Chapman-Enskog` est derived because the impulsion is a conservative local quantity for Navier-Stokes equations. For advection-diffusion equation, this is not any more the case, and it will be necessary to calculate the first order moment of :math:`f_{i}^{(1)}`.

Moments des termes en :math:`\varepsilon`
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The partial derivative in space and times are replaced in :eq:`LBE-Taylor-Chapman-Enskog` by using Eq :eq:`ScalesPartial-Chapman-Enskog` and we also use the expansion of distribution function iwth Eq. 
:eq:`F-Expansion-Chapman-Enskog`. We gather the terms of order :math:`\varepsilon` and :math:`\varepsilon^{2}`.

The gathering of first order terms in :math:`\varepsilon` yields the following equality:

.. math::
   :label: Eq-Ordre1-Flow-Chapman-Enskog

   \partial_{\alpha}^{(1)}e_{i\alpha}f_{i}^{(0)}+\partial_{t_{0}}f_{i}^{(0)}=-\frac{1}{\lambda}f_{i}^{(1)}

With preliminary results Eqs. :eq:`M0-f0-Chapman-Enskog` and :eq:`M1-f0-Chapman-Enskog`, the moment of zeroth-order (sum over
:math:`i`) of that equation is:

.. math::
   :label: M0-Eq-Ordre1-Flow-Chapman-Enskog

   \partial_{t_{0}}\rho+\partial_{\alpha}^{(1)}(\rho u_{\alpha})=0

Similarily, by using Eqs. :eq:`M1-f0-Chapman-Enskog` and :eq:`M2-f0-Chapman-Enskog`, the moment of first order (multiplication of Eq. 
:eq:`Eq-Ordre1-Flow-Chapman-Enskog` by :math:`e_{i\beta}` and sum over :math:`i`) leads to:

.. math::
   :label: M1-Eq-Ordre1-Flow-Chapman-Enskog
...
(Eqs. :eq:`M0-Eq-Ordre1-Flow-Chapman-Enskog` and :eq:`M1-Eq-Ordre1-Flow-Chapman-Enskog`) will be used for calculation of the 2nd order moment of :math:`f_{i}^{(1)}` in the second part of this expansion.
...
.. math::
   :label: TermesOrdre2-Flow-Chapman-Enskog
...
Applying separately :math:`\frac{1}{2}e_{i\beta}\partial_{\beta}` and :math:`\frac{1}{2}\partial_{t_{0}}` on Eq. :eq:`Eq-Ordre1-Flow-Chapman-Enskog` and summing those two results, that Eq. :eq:`TermesOrdre2-Flow-Chapman-Enskog` se simplifies into:
...
With equations :eq:`M0-f0-Chapman-Enskog`, :eq:`M0-fneq-Chapman-Enskog` and :eq:`M1-fneq-Chapman-Enskog`, the moment of order 0 of that equation yields:

.. math::
   :label: M0-Eq-Ordre2-Flow-Chapman-Enskog
...
.. math::
   :label: M1-Eq-Ordre2-Flow-Chapman-Enskog
...
:eq:`M0-Eq-Ordre1-Flow-Chapman-Enskog` + :math:`\varepsilon^{2}\times`\ Eq :eq:`M0-Eq-Ordre2-Flow-Chapman-Enskog`, the moment of order zero is:

.. math::
   :label: ConservMasse-Chapman-Enskog

   \partial_{t}\rho+\partial_{\alpha}(\rho u_{\alpha})=0

which is the first equation of NS system i.e. the mass balance equation. We perform now :math:`\varepsilon\times`\ Eq :eq:`M1-Eq-Ordre1-Flow-Chapman-Enskog` + :math:`\varepsilon^{2}\times`\ Eq :eq:`M1-Eq-Ordre2-Flow-Chapman-Enskog`, the first-order moment is:

.. math::
   :label: ConservQmvt-Chapman-Enskog
...
For :math:`f_{i}^{(1)}` (:math:`\sum_{i}f_{i}^{(1)}e_{i\alpha}e_{i\beta}`), we use the preliminary results :eq:`M2-f0-Chapman-Enskog` and :eq:`M3-f0-Chapman-Enskog` for moments of second- and third-order of EDF :math:`f_{i}^{(0)}`:
...
For last two terms, the time derivative are transformed into space-derivative by using moments of order zero and one of equation of order :math:`\varepsilon` (Eq. :eq:`M0-Eq-Ordre1-Flow-Chapman-Enskog` and :eq:`M1-Eq-Ordre1-Flow-Chapman-Enskog` respectively). For the penultimate term we obtain:

.. math::
   
   -\lambda\partial_{t_{0}}(c_{s}^{2}\rho\delta_{\alpha\beta})=+\lambda c_{s}^{2}\partial_{\gamma}^{(1)}(\rho u_{\gamma})\delta_{\alpha\beta}

That term cancels with one of opposite sign contained in the bracket. For last term, we use the following result:

.. math::
   :label: Equivalence-Chapman-Enskog
...
.. math::
   :label: ViscositeCin-Chapman-Enskog
...
L’équation :eq:`ViscositeCin-Chapman-Enskog` donne la relation entre le paramètre de relaxation :math:`\lambda` de l’équation cinétique de Boltzmann et la viscosité cinématique :math:`\nu` du fluide.
...
En remplaçant :eq:`M2-f0-Chapman-Enskog` et :eq:`Res-Pi1-Chapman-Enskog` dans :eq:`ConservQmvt-Chapman-Enskog`, on obtient :

.. math::
   
   \partial_{t}(\rho u_{\alpha})+\partial_{\beta}\left[c_{s}^{2}\rho\delta_{\alpha\beta}+\rho u_{\alpha}u_{\beta}-\nu(\rho\partial_{\beta}u_{\alpha}+\rho\partial_{\alpha}u_{\beta})\right]+\frac{1}{c_{s}^{2}}\partial_{\beta}\nu\partial_{\gamma}(\rho u_{\alpha}u_{\beta}u_{\gamma})=0

.. math::
   
   \partial_{t}(\rho u_{\alpha})+\partial_{\beta}(\rho u_{\alpha}u_{\beta})=-\partial_{\alpha}(c_{s}^{2}\rho)+\partial_{\beta}\left[\nu(\rho\partial_{\beta}u_{\alpha}+\rho\partial_{\alpha}u_{\beta})\right]+\mathcal{O}(\rho u^{3})
...
En effectuant le développement de Chapman-Enskog (ordre deux en espace, ordre deux en temps et en :math:`\varepsilon^{2}`) et en calculant les
moments d’ordre 0 et 1 des équations en :math:`\varepsilon` et :math:`\varepsilon^{2}`, on a montré que les équations :eq:`LBE-BGK-Chapman-Enskog` et :eq:`Feq-Chapman-Enskog` permettent de résoudre sur un réseau D2Q9 les équations de Navier-Stokes :

.. math::
   :label: Res-Eq1-ChapmanEnskog

   \partial_{t}\rho+\partial_{\alpha}(\rho u_{\alpha})=0

.. math::
   :label: Res-Eq2-ChapmanEnskog
...
.. math::
   :label: Viscosite-Chapman-Enskog
...
.. math::
   ...
   \partial_{t_{0}}(\rho u_{\alpha}u_{\beta})=u_{\alpha}u_{\beta}\partial_{t_{0}}\rho+\rho u_{\alpha}\partial_{t_{0}}u_{\beta}+\rho u_{\beta}\partial_{t_{0}}u_{\alpha}
...
.. math::
   ...
   \partial_{t_{0}}(\rho u_{\alpha}u_{\beta})=-u_{\alpha}u_{\beta}\partial_{t_{0}}\rho+u_{\alpha}\partial_{t_{0}}(\rho u_{\beta})+u_{\beta}\partial_{t_{0}}(\rho u_{\alpha})
...
Un premier développement donne :math:`\partial_{\gamma}^{(1)}(\rho u_{\alpha}u_{\beta}u_{\gamma})=u_{\alpha}\partial_{\gamma}^{(1)}(\rho u_{\beta}u_{\gamma})+\rho u_{\beta}u_{\gamma}(\partial_{\gamma}^{(1)}u_{\alpha})` c’est-à-dire :

.. math::
   :label: Cond1-Chapman-Enskog
...
En effectuant la somme :eq:`Cond1-Chapman-Enskog`, :eq:`Cond2-Chapman-Enskog` et :eq:`Cond3-Chapman-Enskog`, on obtient
:
